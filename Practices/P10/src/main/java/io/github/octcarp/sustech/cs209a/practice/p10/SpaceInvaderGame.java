package io.github.octcarp.sustech.cs209a.practice.p10;
...
public class SpaceInvaderGame extends Application {
    ...
    private void update() {
        ...
        for (Sprite i : EnemyList) {
            if (!i.dead && random.nextInt(1000) < 5) {
                Line temp = shoot(i);
                if (temp != null)
        ...
        for (Line i : PlayerBullet) {
            // player's bullet should move up
            i.setStartY(i.getStartY() - 5);
            i.setEndY(i.getEndY() - 5);
            // should also check whether the bullet hits each enemy
